...
// ComputeOCIChecksum computes the content-addressed checksum for an OCI image
// from its manifest components. The canonical representation is:
//
//	SHA256(config_digest + "\n" + layers_digests_joined_by_newline + "\n" + "linux/" + arch)
//
// Layer digests are joined in manifest order (the OCI spec guarantees immutable
// layer ordering within a manifest). The platform string distinguishes identical
// layer sets built for different architectures.
//
// Returns the full 64-char hex digest and the truncated 16-char checksum.
//
// See docs/04-oci-conversion.md Section 6.2 for the normative contract.
