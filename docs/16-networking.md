...
#### Strategy 1: Cloud-Init (Preferred)

When cloud-init is available in the guest image (which is the case for all standard cloud images), DNS is configured via the network-config section of cloud-init:
...
This is generated by Cocoon from the CNI result and written to the cloud-init metadata source before VM boot.

#### Strategy 2: Kernel Command Line (Fallback)

For images without cloud-init, basic IP configuration can be passed via the kernel `ip=` parameter:

```
ip=10.88.0.5::10.88.0.1:255.255.0.0:myvm:eth0:off nameserver=10.88.0.1
```

This is less flexible (single interface only, limited DNS options) but works with any Linux kernel that has `ip=` boot parameter support.

### 8.3 DNS Configuration in Code

```go
// generateCloudInitNetworkConfig produces a cloud-init network-config v2 YAML
// from the CNI result state.
func generateCloudInitNetworkConfig(attachments []NetworkAttachmentState) ([]byte, error) {
    ...
    for _, att := range attachments {
        ...
        ethernets[att.InterfaceName] = eth
    }
