...
```
root=cocoon-rootfs rootfstype=virtiofs rw [console=...]
```
...
type VMConfig struct {
    ...
    VirtioFSTag   string       `json:"virtiofs_tag,omitempty"`    // virtiofs rootfs tag (canonical: cocoon-rootfs)
...
- [x] **Direct Kernel Boot** (OCI VM images) — implemented (resolver auto-detect + registry auto-pull runtime path):
  ...
  - [x] Build kernel cmdline with `root=cocoon-rootfs rootfstype=virtiofs rw` (+ console entries)
